...
## The topics
A topic is an independent group of appointments. Topics are identified by their name, which must be valid as part of PostgreSQL names for tables and indexes. Appointments of separate topics are never polled together, so each topic needs a dedicated scheduler (see *The scheduler* section below).
    
## The appointments 
The appointments are the main objects handled in Ajenda. They are the messages to be delivered in the specified date/time.     
### One-shot appointments In its simplest form, an appointment consists on    
...
- `withUid(UUID appointmentUid)`: Explicitly sets the UUID of the appointment    
   - `withHashUid(String key)`: Builds a type-5 UUID from the given key (which uses the SHA-1 algorithm to build a hash).
      - The same UUID can be reconstructed using `UUIDType5.nameUUIDFromCustomString(key)`, making it handy for recalling UUID from more domain-relevant identifiers and for later cancellation.
...
## The scheduler  
  `AjendaScheduler` is the class which gathers the handling of a single topic and its appointments on their four general operations:
 - **Topic initialization**: Invisible to the user, it happens whenever a new scheduler is created.  
 - **Booking**: This is the action of inserting a new appointment.  
 - **Listening and handling**: Waiting for appointments to reach ther due date and handling them when they arrive.
 - **Cancelling**: Removing future appointments or aborting periodic ones.
  
### Topic initialization
Every topic makes use of two tables, one for appointments (which is also indexed by due date) and another for periodic appointments specifications. These tables are created, if they do not exist, whenever a scheduler for a topic is intantiated. Expected names for those objects are:  
 - Table **ajenda_*topic***
 - Table **periodic_ajenda_*topic***
 - Index **idx_ajenda_*topic*_duedate**
 
The average user should not need tinkering with the initialization. For advanced use see *Advanced use* section below.

### Booking
Adding a new appointment is a straightforward operation once the `Appointment` or `PeriodicAppointment` object is built:
- `book(AppointmentBooking... bookings)`: Adds all the appointments to the agenda. This operation is transactional, which means that if any error occurs during the booking process, no appointment of the list will remain booked. As explained in the *The appointments* section, in case of UUID conflict the previously existing appointment is overwritten.
- `book(List<AppointmentBooking> bookings)`: A variant of the previous method, with the same behaviour.
- `bookPeriodic(PeriodicAppointmentBooking... bookings)`: Adds all the periodic appointments to the agenda as well as their N first iterations, being N the respective values of key iteration. This operation is transactional. As explained in the *The appointments* section, in case of periodic appointments UUID conflict an exception will be thrown.
- `bookPeriodic(List<PeriodicAppointmentBooking> bookings)`: A variant of the previous method, with the same behaviour.
- `tryBookPeriodic(PeriodicAppointmentBooking... bookings)`: Differs from `bookPeriodic` in the way of handling conflicts, when this method applies a *book if not exists* policy.
- `tryBookPeriodic(List<PeriodicAppointmentBooking> bookings)`: A variant of the previous method, with the same behaviour.

### Listening and handling
Listening for due appointments is done by the seemingly simple mechanism of polling the topic appointments table. However, there are many parameters which can affect this process and the interweaving with the handling of the polled appointments.
#### Poll and queue size
#### Concurrency level
#### Periodicity and look-ahead
#### Clock
#### Life-cycle model